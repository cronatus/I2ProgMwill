		...
		while(temp1 != 1){
			...
			startingloc();
				...
				if(turn >= 26){																							//The code within this statement
					...
					TextIO.putln("Congratulations you have completed the Knights Tour on a 5x5 board");					//in this it simply detects that the
...
//-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-	
	...
	public static void startingloc(){
		...
		input = TextIO.getln() + "1";	//Variable for the users input  to be used and separated below
			...
			case "A":							
				File = 1;						// A Realisation while helping others with this project showed that using a Char instead of a
				break;							// String would be more efficient and require less code.
			case "b":
			case "B":
				File = 2;
				break;
